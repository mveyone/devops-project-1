node{
    ...
    stage('copy files from ansible to kubernetes server'){
        sshagent(['ansible_demo']){
                sh 'ssh -o StrictHostKeyChecking=no ubuntu@172.31.18.136 '
                sh 'scp /var/lib/jenkins/workspace/devops-aws/*  ubuntu@172.31.18.136:/home/ubuntu'   
                sh 'ssh -o StrictHostKeyChecking=no ubuntu@172.31.18.136 unzip -o devops-jenkins-aws-k8s-ansible.zip ' 
                sh 'ssh -o StrictHostKeyChecking=no ubuntu@172.31.18.136 ansible-playbook k8s/k8s-playbook.yml'   
                   }
    }
}    